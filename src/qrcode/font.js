...
const emptyChar = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,]

const getFonts = (text) => {
  if (!text) { return [] }

  return text.split('').map(c =>
    (font[c.charCodeAt(0) - initIndex] || emptyChar)  // array of dot matri lines
      .map(line => (line << 1).toString(2).padStart(6, '0')      // convert to binary
        .split('')                        // split to array
        .map(x => parseInt(x))            // convert to array of bit
      )
  )
}
